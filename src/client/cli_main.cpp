...
QJsonValue typedValue(const QString& key,const QString& text) {
    ...
    if(QStringList{"rhset","aqil","dt"}.contains(key)) {
        bool ok=false; const int value=text.toInt(&ok); if(ok) return value;
    }
    return text;
}
...
int main(int argc,char** argv) {
    ...
    bool timeoutOk=false; const int timeout=parser.value("timeout").toInt(&timeoutOk);
    bool portOk=false; const uint port=parser.value("port").toUInt(&portOk);
    const QString host=parser.value("host").trimmed();
    ...
    try {
        ...
        if(command=="set") {
            ...
            for(int i=1;i<positional.size();++i) {
                const qsizetype split=positional[i].indexOf('=');
                if(split<1) throw std::runtime_error("set expects KEY=VALUE");
                const QString key=positional[i].left(split);
        ...
        QByteArray buffer;
        for(;;) {
            const QJsonObject envelope=nextLine(socket,buffer,timeout*1000);
