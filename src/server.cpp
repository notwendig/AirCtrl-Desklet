...
bool loadConfig(const QString& path,ServerConfig* config,QString* error) {
    ...
    const QString addressText=file.value("server/listen_address","0.0.0.0").toString().trimmed();
    QHostAddress address;
    bool listenPortOk=false,devicePortOk=false,reconnectOk=false,requestOk=false,idleOk=false;
    const uint listenPort=file.value("server/port",5680).toUInt(&listenPortOk);
    DeviceConfig device;
...
private:
    ...
    void acceptClients() {
        while (QTcpSocket* socket = listener_.nextPendingConnection()) {
            ...
            connect(socket, &QTcpSocket::disconnected, this, [this, id] {
                if (QTcpSocket* old = clients_.take(id)) old->deleteLater();
                buffers_.remove(id);
    ...
    void readClient(quint64 client) {
        QTcpSocket* socket = clients_.value(client);
        if (!socket) return;
    ...
    void handle(quint64 client, const QJsonObject& request) {
        const QString kind = request.value("_airctrl").toString();
        if (kind == "configure") {
        ...
        if (kind == "control") {
            const qulonglong id = request.value("id").toVariant().toULongLong();
            const QJsonObject values = request.value("values").toObject();
    ...
    void broadcast(const QJsonObject& object) {
        const QList<quint64> ids = clients_.keys();
        for (const quint64 id : ids) send(id, object);
    }
    ...
    void postStatus(const aioairctrl::Json& status) {
        const QByteArray bytes = QByteArray::fromStdString(status.dump());
            ...
            try {
                ...
                while (!stopping_.load()) {
                    ...
                    if (takeCommand(&command)) {
                        ...
                        try {
                            const aioairctrl::Json json = aioairctrl::Json::parse(
...
int main(int argc, char** argv) {
    ...
    const QString testConfig=qEnvironmentVariable("AIRCTRL_TEST_SERVER_CONFIG").trimmed();
